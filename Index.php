    ...
    function awesome_ssl() {
        ...
        if ( ! isset( $awesome_ssl ) ) {
            ...
            $awesome_ssl = fs_dynamic_init( array(
                ...
                'secret_key'          => 'sk_7S&TdEYoyyGOXW?j4Q-{Od]b@Ya)N',
            ) );
        }
 ...
 */
  ...
  <tr>
    <td>
				...
				?></td>
    <td></td>
    <td><div class="sslvalidnotice" style="background-color:lightgreen; text-align:center; font-weight: bold;"><h1 id="titlesslvalid"></h1><p id="textsslvalid"></p><p id="validdomain"></p></div>
<div class="sslinvalidnotice" style="background-color:red; text-align:center; font-weight:bold;"><h1 id="titlesslinvalid"></h1> <p id="textsslinvalid"></p><p id="invaliddomain"></p></div>
<script>
if (location.protocol !== 'https://') {
document.getElementById("titlesslvalid").innerHTML = "SSL Enabled!";
document.getElementById("textsslvalid").innerHTML = "A valid SSL Certificate has been found on this website. SSL is now enabled.";
document.getElementById("validdomain").innerHTML = 
"Current Domain: https://" + window.location.hostname;
} else {
document.getElementById("titlesslinvalid").innerHTML = "SSL Invalid!";
document.getElementById("textsslinvalid").innerHTML = "No SSL Certificate was found. Get one from Lets' Encrypt or your hosting company to get started.";
document.getElementById("invaliddomain").innerHTML = 
"Current Domain: http://" + window.location.hostname;
}
</script><div class="checkSSL">
...
<?php submit_button(); ?>
				<input type="button" id="checkConnectionbtn" style="display:none;" value="Check SSL" onclick="alert('Your Site Is Connected To HTTPS, SSL Is Now <strong>Enabled</strong>')">
			</form>
			</div>
		</div>
	<?php }
	...
	public function force_site_to_run_through_https_on_all_pages_2_callback() {
		printf(
			'<div class="parent-checkbox"><input type="checkbox" name="ssl_option_name[force_site_to_run_through_https_on_all_pages_2]" id="force_site_to_run_through_https_on_all_pages_2" value="force_site_to_run_through_https_on_all_pages_2" disabled%s></div>',
			( isset( $this->ssl_options['force_site_to_run_through_https_on_all_pages_2'] ) && $this->ssl_options['force_site_to_run_through_https_on_all_pages_2'] === 'force_site_to_run_through_https_on_all_pages_2' ) ? '' : ''
		);
	}
